...
First load the favourite numbers numbers csv file that you've downloaded from github
```{r}
favouriteNumbers <- read.csv("favouriteNumbers.csv")
```
...
```{r}
smallerNumbers <- favouriteNumbers %>% subset(numbers >= 0 & numbers <= 20)
```
...
```{r}
favouriteNumbers$numbers %>% table() %>% as.data.frame() %>%  write.csv(file="frequencyTable.csv")
```
