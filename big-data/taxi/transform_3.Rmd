...
```{r}
clean_taxis <- taxis %>% ___(___, RateCodeID ==1)
```
...
```{r}
pickups <-  clean_taxis %>% ___(___ = ___) %>% ___(___) %>% ___(pickups = n()) %>% ___(___)
...
```{r}
pickdrops_spark <- ___(___, ___, by="___") %>%  ___(___)
```
