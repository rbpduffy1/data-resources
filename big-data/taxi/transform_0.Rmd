...
So, we only need the pick up locations (PULocationID) and drop off locations (DOLocationID) from now on, so just select those columns.
```{r}
where_taxis <- clean_taxis %>% select(PULocationID, DOLocationID)
```
...
We need to
- select only the PULocationID column and give it a new name `LocationID`
...
```{r}
pickups <-  where_taxis %>% select(LocationID = PULocationID) %>% group_by(LocationID) %>% summarise(pickups = n()) %>% arrange(LocationID)
