...
head(loan)
# Anyone guess what the command for getting the last 6 rows is?
# tail(loan)
```
```{r}
...
Replace them with Males because they're the majority
Maybe it's people who don't want to declare their gender - should we create an 'Unspecified' value? Not Recommended
```{r}
...
Delete rows with NAs -- NAs make no sense, i.e. it's either Yes or No (1 or 0)
Replace with 0
```{r}
...
Delete rows with NAs
Replace NAs with 0s -- i.e. NA dependents = 0 dependents
Possibly assume people don't want to declare and create new attribute value?
```{r}
# Check unique values of Dependents
summary(loan$Dependents)
```
```{r}
# Let's say we assume that NA = 0, and replace appropriately. Note, we're replacing with '0' as a character, not 0 
# as a number. 0 would create a new attribute. 
loan$Dependents[is.na(loan$Dependents)] <- "0"
# Check 
colSums(is.na(loan))
```
Deal with Self-employed -- options:
Delete rows with NAs -- not safe to assume anything. Also as a bank, this maybe essential info to make a judgement on loan issuing
Assume NA = people are not self-employed
Assume NA = people are self-employed? Maybe some interesting justification for this for financial services clients
```{r}
...
Delete rows with NAs -- after all we need a loan amount in order to issue a loan!
Replace with 0
```{r}
...
Delete rows with NAs -- after all don't we need a term amount in order to issue the loan? (common sense)
Replace with 0?
```{r}
...
Note You can skip to this instead of going through the above if you are short on time.
